...
describe("contextテスト、Prefecture Hook", () => {
  ...
  test("useGetIdFromNameFunc| 名前からIDを取り出せるかテスト", () => {
    ...
    const { result, rerender } = renderHook(
      (props) => useGetIdFromNameFunc({ ...props }),
      {
    ...
    validate(result.current, name2Id); //一部undefined

    rerender({ ...new_name2Id });
    validate(result.current, new_name2Id);
  });

  test("useGetNameFromIdFunc| IDから名前を取り出せるか", () => {
    ...
    const { result, rerender } = renderHook(
      (props) => useGetNameFromIdFunc({ ...props }),
      { initialProps: name2Id }
    );
